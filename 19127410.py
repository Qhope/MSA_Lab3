...
#     data.loc[:, :"LSTAT"], diagonal="kde", figsize=(20, 15))
# plt.show()
columnName = ['CRIM', 'ZN', 'INDUS', 'CHAS', 'NOX', 'RM',
              'AGE', 'DIS', 'RAD', 'TAX', 'PTRATIO', 'B', 'LSTAT', 'MEDV']

# for i in range(len(columnName)-2):
#     sns.lmplot(x=columnName[i], y=columnName[i+1],
#                data=data, hue="MEDV", fit_reg=False)
# plt.show()

print(columnName[:columnName.index("MEDV")])

ax = data[columnName[:columnName.index("MEDV")]].plot(figsize=(20, 15))
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))

ax = data[['CRIM', 'ZN', 'INDUS', 'CHAS', 'NOX']].plot(figsize=(20, 15))
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))

ax = data[['RM','AGE', 'DIS', 'RAD', 'TAX']].plot(figsize=(20, 15))
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))

ax = data[['PTRATIO', 'B', 'LSTAT']].plot(figsize=(20, 15))
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))

ax = data[['PTRATIO', 'B']].plot(figsize=(20, 15))
ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
#plt.show()
...
print("## CRIM_W:")
print(calc_within_groups_variance(X.CRIM, y))
...
print("## CRIM_b:")
print(calc_between_groups_variance(X.CRIM, y))
...
def calc_separations(variables, group_variable):
    ...
    for variable_name in variables:
        ...
        sep = v_b / v_w
        print("variable", variable_name, "Vw=",
              v_w, "Vb=", v_b, "separation=", sep)
...
print("## cov_w:")
print(calc_within_groups_covariance(X.CRIM, X.LSTAT, y))
...
def calc_between_groups_covariance(variable1, variable2, group_variable):
    ...
    for level_i in levels:
        ...
        term1 = (mean1 - variable1mean) * \
            (mean2 - variable2mean) * level_i_length
        cov_b += term1
    cov_b /= num_levels - 1
    return cov_b
...
print("## cov_b:")
print(calc_between_groups_covariance(X.CRIM, X.LSTAT, y))

# calculating correlations for multivariate data
corr = stats.pearsonr(X.AGE, X.TAX)
...
def most_highly_correlated(my_dataframe, num_to_report):
    ...
    cor_matrix = cor_matrix.reindex(
        cor_matrix.abs().sort_values(ascending=False).index).reset_index()
...
def pca_summary(pca, standardised_data, out=True):
    ...
    c = [np.sum(pca.explained_variance_ratio_[:i])
         for i in range(1, len(pca.explained_variance_ratio_)+1)]
